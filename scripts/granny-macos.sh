...
if [[ "${OS_NAME}" != "Darwin" ]]; then
  die "This installer currently supports macOS only. Detected: ${OS_NAME}"
fi

say "Lantern Easy Install (macOS)"
...
case "${ARCH}" in
  x86_64)
    SELECTED_TARBALL="${MACOS_X86_64_TARBALL:-}"
    SELECTED_SHA="${MACOS_X86_64_TARBALL_SHA256:-}"
    ;;
  arm64|aarch64)
    SELECTED_TARBALL="${MACOS_AARCH64_TARBALL:-}"
    SELECTED_SHA="${MACOS_AARCH64_TARBALL_SHA256:-}"
    ;;
  *)
...
[[ -n "${SELECTED_TARBALL}" ]] || die "Missing macOS tarball metadata for ${ARCH}"
[[ -n "${SELECTED_SHA}" ]] || die "Missing macOS checksum metadata for ${ARCH}"
...
ASSET_URL="${REPO_RAW_BASE}/releases/${VERSION}/${SELECTED_TARBALL}"
DOWNLOAD_PATH="${WORK_DIR}/${SELECTED_TARBALL}"

say "Downloading ${SELECTED_TARBALL} ..."
...
if [[ "${ACTUAL_SHA}" != "${SELECTED_SHA}" ]]; then
  die "Checksum mismatch. Expected ${SELECTED_SHA}, got ${ACTUAL_SHA}"
fi
